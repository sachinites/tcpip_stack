...
isis_show_one_interface_group (node_t *node, isis_intf_group_t *intf_grp) {
    ...
    rc = sprintf(buff, "Intf-grp name : %s\n", intf_grp->name);
    rc += sprintf (buff, "\tMember Interfaces : ");

    ITERATE_GLTHREAD_BEGIN(&intf_grp->intf_list_head, curr) {

        intf_info = intf_grp_member_glue_to_intf_info(curr);
        rc += sprintf (buff, "%s  ", intf_info->intf->if_name);
...
isis_show_all_interface_group(node_t *node) {
    ...
    ITERATE_AVL_TREE_BEGIN(&isis_node_info->lspdb_avl_root, avl_node) {

        intf_grp = avltree_container_of(avl_node, isis_intf_group_t, avl_glue);

        rc += isis_show_one_interface_group(node, intf_grp);
