...
l3rib_lookup_lpm(rt_table_t *rt_table, 
    ...
    *default_l3_rt = NULL;
    ...
    char subnet[IP_LEN];
    char dest_ip_str[IP_LEN];
    char longest_mask = 0;
   
    dest_ip = htonl(dest_ip); 
    inet_ntop(AF_INET, &dest_ip, dest_ip_str, IP_LEN);
    dest_ip_str[IP_LEN - 1] = '\0';
     
    ITERATE_GLTHREAD_BEGIN(&rt_table->route_list, curr){

        l3_route = rt_glue_to_l3_route(curr);
        memset(subnet, 0, IP_LEN);
        apply_mask(dest_ip_str, l3_route->mask, subnet);

        if(strncmp("0.0.0.0", l3_route->dest, IP_LEN) == 0 &&
                l3_route->mask == 0){
...
is_layer3_local_delivery(node_t *node, unsigned int dst_ip)
{
    ...
    inet_pton(AF_INET, node->node_nw_prop.lb_addr.ip_addr, &addr_int);
    dst_ip = htonl(dst_ip);
    if(dst_ip == addr_int)
        return TRUE;
    ...
    for(int i=0; i<MAX_INTF_PER_NODE && node->intf[i]; i++)
    {
        ...
        {
            inet_pton(AF_INET, node->intf[i]->intf_nw_props.ip_add.ip_addr, &addr_int);
            if(dst_ip == addr_int)
                return TRUE;
        }
    }
...
{
    layer3_pkt_recv_from_bottom(node, recv_intf, pkt, pkt_size, L3_protocol_number);
}
...
demote_pkt_to_layer3(node_t *node,
                     char *pkt, unsigned int pkt_size,
                     int protocol_number, u_int32_t dest_ip_addr)
{
    layer3_pkt_receieve_from_top(node, pkt,
        pkt_size, protocol_number, dest_ip_addr);    
}