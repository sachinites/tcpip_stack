...
static const char *
bdrmac_path_str(bool recvd_on_overlay)
{
    return recvd_on_overlay ? "overlay" : "underlay";
}
                              ...
                              uint32_t ctx)     // bd port
{
    ...
    if (recv_intf == intf)
    {
        /* Self Generated pkt bounced back, ignore . For example, ARP-B request
            generated by the Router itself would feed ingress interface as
            RMAC interface in pkt's pvt data, and then flooding would bounce 
            back it to RMAC interface again */
        pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD | DFLOW,
            "BDRmac %s : Ignoring self-generated pkt %s bounced back from flood\n",
    ...
    pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD,
        ...
        pkt_mbuf_str(mbuf), 
        intf->if_name, 
        recv_intf ? recv_intf->if_name : "None", bd_intf->if_name);
    ...
    if (is_pkt_vlan_tagged(eth_hdr)) {
        pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD|DERR,
            ...
            pkt_mbuf_str(mbuf), intf->if_name, 
            recv_intf ? recv_intf->if_name : "None");
    ...
    pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD_DET,
        ...
        bd_intf->if_name, 
        recv_intf ? recv_intf->if_name : "None", pkt_mbuf_str(mbuf));

    vrf = bd_intf->vrf;

    /* recv_intf can be NULL when pkt is routed from BD1 to BD2 on same Router 
        (recv_intf is cleared on routing), 
        and ARP is resolved for destination already 
        and MAC is not learned for destination then pkt is flooded
        in BD2 (as Unknown unicast) and therefore, bdrmacif would going to recv 
        pkt without recv_intf. There is no MAC learning without BGP-EVPN control 
        plane when MPLS/SRv6 Transport is used in core !
    */
    ...
    if (!recvd_on_overlay && recv_intf) {
        assert (recv_intf->ac_intf);
        recv_intf = recv_intf->ac_intf;
    }

    if (!recv_intf) {

        /* This happens when pkt traverse down from L3 to L2 and if it is 
            unknown unicast */
        pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD | DFLOW,
            "BDRmac BD %s : recvd Pkt with no recv-intf, pkt dropped\n", 
            bd_intf->if_name);
        return 0;
    }

    pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD | DFLOW,
        "BDRmac BD %s : path=%s recv_intf=%s eth_type=0x%04x\n",
        bd_intf->if_name,
        bdrmac_path_str(recvd_on_overlay),
        recv_intf ? recv_intf->if_name : "None",
        (unsigned)ntohs(eth_hdr->type));

    /*
     * Validate pkts generated by RMAC interface from other PE routers.
     * Check for Duplicate IPs on SVI without distributed anycast gateway MAC.
     * Implemented line-by-line per case — do not merge subcases.
     */

    if (ntohs(eth_hdr->type) == ETH_TYPE_ARP) {

        arp_hdr_t *arp_hdr_in = (arp_hdr_t *)GET_ETHERNET_HDR_PAYLOAD(eth_hdr);
        uint16_t arp_op = ntohs(arp_hdr_in->op_code);
        uint32_t arp_src_ip = ntohl(arp_hdr_in->src_ip);
        uint32_t arp_dst_ip = ntohl(arp_hdr_in->dst_ip);

        /* Case 1 : ARP-B */
        if (arp_op == ARP_BROAD_REQ && recvd_on_overlay) {

            /* case 1.1 : Recvd over Overlay */

            /* if Src ip == My IP → Duplicate IP Address, drop the pkt */
            ...
            /* If Dst ip == My ip → drop (do not reply over overlay broadcasts) */
            if (bd_intf->ip_addr && arp_dst_ip == bd_intf->ip_addr) {
        ...
        if (arp_op == ARP_BROAD_REQ && !recvd_on_overlay && recv_intf) {

            /* case 1.2 : Recvd over Underlay */
            ...
            /* If Dst ip == My ip → Reply with ARP reply */
            if (bd_intf->ip_addr && arp_dst_ip == bd_intf->ip_addr) {
        ...
        /* Case 2 : ARP-Reply pkt */
        if (arp_op == ARP_REPLY && recvd_on_overlay) {

            /* case 2.1 : Recvd over Overlay */
            ...
            if (bd_intf->ip_addr && arp_dst_ip == bd_intf->ip_addr) {
                ...
                /* If Dst Mac == My Mac → drop (stay silent) */
                if (mac_address_compare(arp_hdr_in->dst_mac.mac, bd_intf->mac_add.mac) ||
                    mac_address_compare (dp_ctx->anycast_gw_mac.mac, arp_hdr_in->dst_mac.mac)) {
                    
                    /* ARP Learning*/
                    arp_table_update_from_arp_pkt(dp_ctx, 
                                              vrf, vrf->arp_table,
                                              arp_hdr_in, recv_intf);  

                    pkt_tracer(mbuf, dp_ctx->dptr, DARP | DFLOW,
                        "BDRmac BD %s : Recvd ARP Reply for Self, Pkt Dropped\n",
                        bd_intf->if_name);
        ...
        if (arp_op == ARP_REPLY && !recvd_on_overlay && recv_intf) {

            /* case 2.2 : Recvd over Underlay */
        ...
        pkt_tracer(mbuf, dp_ctx->dptr, DARP | DERR,
            "BDRmac BD %s : unsupported ARP op=%u, drop\n",
            bd_intf->if_name, (unsigned)arp_op);
        intf->recvd_pkt_dropped++;
        return 0;
    }

    /* Case 3 : Any other unicast ethernet pkt */
    if (recvd_on_overlay) {

        /* case 3.1 : Recvd over Overlay */

        /* if dst mac == my mac → promote to L3 */
        if (mac_address_compare((unsigned char *)bd_intf->mac_add.mac,
                                (unsigned char *)eth_hdr->dst_mac.mac)||
            mac_address_compare ((unsigned char *)dp_ctx->anycast_gw_mac.mac, 
                                (unsigned char *)eth_hdr->dst_mac.mac)) {

            gen_proto_id_t proto = ntohs(eth_hdr->type);
            pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD | DFLOW,
                "BDRmac BD %s : Recvd unicast ethernet packet on overlay tunnel, Promote to L3\n",
                bd_intf->if_name, (unsigned)proto);
        ...
        /* else → Drop the pkt */
        pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD | DERR,
            "BDRmac BD %s : case 3.1 — overlay dropped pkt %s (dst mac != rmac)\n",
            bd_intf->if_name, pkt_mbuf_str(mbuf));
        intf->recvd_pkt_dropped++;
        return 0;
    }

    /* case 3.2 : Recvd over Underlay */

    /* if dst mac == my mac → promote to L3 */
    if (recv_intf &&
        (mac_address_compare((unsigned char *)bd_intf->mac_add.mac,
                            (unsigned char *)eth_hdr->dst_mac.mac) ||
         mac_address_compare((unsigned char *)dp_ctx->anycast_gw_mac.mac,
                            (unsigned char *)eth_hdr->dst_mac.mac))) {
        ...
        gen_proto_id_t proto = ntohs(eth_hdr->type);
        pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD | DFLOW,
            "BDRmac BD %s : Recvd unicast ethernet packet on local AC, Promote to L3\n",
            bd_intf->if_name, (unsigned)proto);
    ...
    /* else → Drop the pkt */
    pkt_tracer(mbuf, dp_ctx->dptr, DL2FWD | DERR,
        "BDRmac BD %s : Unrecognized pkt recvd, dropped\n",
        bd_intf->if_name, pkt_mbuf_str(mbuf));
    intf->recvd_pkt_dropped++;
    return 0;
}
    ...
    if (ntohs(vlan_eth_hdr->type) == ETH_TYPE_ARP) {
        ...
        if (ntohs(arp_hdr_in->op_code) == ARP_BROAD_REQ) {
            ...
            if (vlan_intf->ip_addr == ntohl(arp_hdr_in->dst_ip)) {
                ...
                mac_addr_t *reply_src_mac =
                    dp_arp_gateway_reply_src_mac(dp_ctx, &vlan_intf->mac_add);
                 ...
                 ntohl(arp_hdr_in->dst_ip) == vlan_intf->ip_addr)
        {
            if (mac_address_compare(arp_hdr_in->dst_mac.mac, vlan_intf->mac_add.mac) || 
                mac_address_compare((unsigned char *)dp_ctx->anycast_gw_mac.mac, 
                                    (unsigned char *)arp_hdr_in->dst_mac.mac))
            {
        ...
        for (p = 0; p < th_data->n_ports; p++) {
            ...
            for (uint16_t q_id = 0; q_id < dp_intf->dpdk_max_rx_queues; q_id++) {
                ...
                for (i = 0; i < nb_rx; i++) {
                    ...
                    pkt_mbuf_clear_ingress_intf(mbuf);
                    dp_pkt_entry_point (dp_intf->dp_ctx, dp_intf->vrf, dp_intf, mbuf, 0);
                    pkt_mbuf_dereference(mbuf);
                }
            }
        }
